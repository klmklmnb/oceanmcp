...
默认情况下，SDK 连接到 `http://ssoapitest.mihoyo.com/ocean-mcp-api-server` 上的 OceanMCP API 服务器。在生产或测试环境中，你可以将 SDK 指向你实际部署的服务器地址。

服务器地址按以下优先级解析：

1. **运行时覆盖** — `window.__OCEAN_MCP_SERVER_URL__`（最高优先级）
2. **构建时环境变量** — `VITE_API_URL`（Vite 构建时写入）
3. **兜底默认值** — `http://ssoapitest.mihoyo.com/ocean-mcp-api-server`
