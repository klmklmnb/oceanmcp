...
| 字段        | 类型     | 默认值                           | 说明                                                                         |
| ----------- | -------- | -------------------------------- | ---------------------------------------------------------------------------- |
| `default`   | `string` | 服务器 `LLM_MODEL` 环境变量      | 主模型 ID（如 `"gpt-4o"`、`"claude-sonnet-4-20250514"`、`"glm-4.6"`）。 |
...
默认情况下，SDK 连接到 OceanMCP API 服务器。在生产或测试环境中，你可以将 SDK 指向你实际部署的服务器地址。
...
> **注意：** ESM 的 `.d.ts` 文件引用了 `oceanmcp-shared` 中的类型。如果你通过 npm/pnpm 安装 `@ocean-mcp/frontend-sdk`，shared 包会作为依赖自动拉取，不需要额外操作。
