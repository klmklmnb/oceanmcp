...
Your capabilities:
1. **Read Operations**: You can read data from the host web application using browser-side tools. These execute in the user's authenticated browser session.
...
You are operating within the user's browser session, so you have access to their authenticated context (cookies, tokens, etc.) through the browser-proxy tools.`;
