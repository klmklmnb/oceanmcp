...
export const imageOcr = tool({
  ...
  inputSchema: z.object({
    model: z
      ...
      .optional()
      .describe(
        "Vision model ID to use (e.g. mihoyo-qwen3-vl-235b-fp8). Defaults to the configured LLM_MODEL.",
      ),
    prompt: z
      .string()
      .optional()
  ...
  execute: async ({ model, prompt, imageUrl }) => {
    const llm = getLanguageModel(model ?? "mihoyo-qwen3-vl-30b-a3b-instruct-fp8");
