...
describe("deduplicateAssistantParts", () => {
  ...
  describe("duplicate tool parts", () => {
    test("removes duplicate userSelect with identical input", () => {
      ...
      const first = userSelectPart(selectInput, { selectedValue: "ocean" });
    ...
    test("removes duplicate non-userSelect tool parts with same input", () => {
      const tool1 = toolPart("listAppClustersocean", {});
      const tool2 = toolPart("listAppClustersocean", {});
    ...
    test("keeps tool parts of different types even with same input shape", () => {
      const tool1 = toolPart("listAppClustersocean", {});
      const tool2 = toolPart("listAppClusterssea", {});
  ...
  describe("real-world reproduction: userSelect duplication bug", () => {
    test("removes duplicate userSelect cards and text from multi-step conversation", () => {
      const tenantSelectInput = {
        ...
        options: [
          { value: "ocean", label: "ocean（已创建 testing 集群 cluster_id=8217）" },
          { value: "sea", label: "sea（需先创建 testing 集群）" },
        ],
      };
      ...
      const responseText =
        "继续在 **ocean** 做（testing 集群 `cluster_id=8217`），bucket_tag 选 **intranet**。\n\n请提供 domain 和 public_path。";
      ...
      const msg = assistantMsg([
        ...
        toolPart("loadSkill", { name: "devops" }),
        stepStart(),
        toolPart("listAppClustersocean", {}),
        toolPart("listAppClusterssea", {}),
        stepStart(),
        userSelectPart(
          {
            message: "未在 sea 租户下查到任何集群…",
            options: [
              { value: "ocean:pre", label: "ocean 预发(pre)" },
              { value: "ocean:create_testing", label: "在 ocean 创建测试集群(testing)" },
            ],
          },
          { selectedValue: "ocean:create_testing" },
        ),
        stepStart(),
        toolPart("executePlan", {
          intent: "创建 testing 集群",
          steps: [{ functionId: "createClusterocean" }],
        }),
        stepStart(),
        userSelectPart(
          {
            message: "ocean testing 集群已创建，下一步做什么？",
            options: [
        ...
        stepStart(),
        userSelectPart(tenantSelectInput, { selectedValue: "ocean" }),
        stepStart(),
        ...
        stepStart(),
        userSelectPart(tenantSelectInput, { selectedValue: "ocean" }),  // dup
        stepStart(),
      ...
      // 1. env question ("未在 sea 租户下查到任何集群…")
      // 2. next-step question ("ocean testing 集群已创建…")
  ...
  describe("edge cases", () => {
    ...
    test("tool parts with same tool name but different input are preserved", () => {
      ...
      const tool2 = toolPart("executePlan", {
        intent: "Deploy group",
        steps: [{ functionId: "deployGroupocean" }],
      });
