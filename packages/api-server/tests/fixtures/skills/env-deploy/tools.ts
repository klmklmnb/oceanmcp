/**
 * Environment deployment tools — demonstrates userSelect integration.
 *
 * These tools are Vercel AI SDK Tool format. They showcase a pattern where
 * a tool's execute() triggers the `userSelect` tool by returning a
 * structured result that tells the LLM to call userSelect next.
 *
 * In practice, the LLM decides to call userSelect based on the tool's
 * description and the ambiguity of the user's request. These tools
 * provide the option metadata for the LLM to pass to userSelect.
 */
import { tool } from "ai";
import { z } from "zod";
...
export default {
  /**
   * Deploy tool — the LLM is instructed to call userSelect first
  ...
  deploy: tool({
    description:
      ...
      "If the user has not specified an environment, call userSelect first " +
    ...
    execute: async ({ environment, version }) => {
      ...
      if (!validEnvs.includes(environment)) {
        return {
          ...
          hint: "Call userSelect with the valid environment options.",
        };
      }
      return {
        status: "deployed",
   ...
   * which should trigger a dropdown in Wave instead of buttons.
   */
  rollback: tool({
    description:
      "Rollback a deployment to a previous version. " +
      "If the user has not specified a version, call userSelect first " +
