...
| `annualLeaveInfo` | string | 年假到期/过期提醒（如"其中0.5天将于3-21过期"） |
...
|------|------|----------|-------------|
| `leaveType` | string | yes | 假期类型代码，从 `leaveTypeList` 中选择。支持 `"AnnL"`（年假）、`"SicLM"`（带薪病假）、`"SicL"`（普通病假）。用户未指定时默认年假 |
...
- **年假到期提醒** (`annualLeaveInfo`)，如"其中0.5天将于3-21过期" — 如果该字段有值，必须醒目地展示给用户
...
在对话中向用户确认以下内容：
1. **假期类型** — 如用户明确说"请病假"，使用 `SicLM`（带薪病假）或 `SicL`（普通病假）；如用户未指定或说"请假"，默认使用 `AnnL`（年假）
2. **请假起止日期** — 直接用文字询问用户，如"请问您想请几号到几号的假？是上午还是下午还是全天？"。时间只有三个时间点可选：**10:00**（上午开始）、**15:00**（下午开始）、**19:00**（当天结束）
3. **请假原因** — 直接问"请假原因是什么？"
...
2. **假期类型支持年假和病假**：`AnnL`（年假）、`SicLM`（带薪病假）、`SicL`（普通病假）。用户未明确指定时默认使用年假。如果用户说"请病假"，需确认是带薪病假还是普通病假（优先使用 `SicLM`）。
3. **通过对话文字直接询问用户日期和原因，不要渲染表单组件**。通知人自动填充（`informedPerson` 或 `parentDomain`），不需要询问用户。
...
7. **年假到期提醒**：如果 `annualLeaveInfo` 有值（如"其中0.5天将于3-21过期"），必须在展示余额时一并告知用户，提醒用户注意即将过期的年假。
